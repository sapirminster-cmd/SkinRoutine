  ...
  <div class="main-scroll">
    ...
    <div class="view" id="view-products">
      ...
      <div style="padding:1rem 1rem .5rem">
        <h1 style="font-family:'DanaYad',sans-serif;-webkit-text-stroke:.8px var(--text-dark);
                   font-size:1.45rem;color:var(--text-dark);margin:0">ספריית מוצרים</h1>
        <span id="products-count" style="font-size:.7rem;color:var(--text-soft);display:block;margin-top:.1rem"></span>
      </div>

      <!-- Row 2: Feature buttons -->
      <div style="display:flex;gap:.4rem;padding:0 1rem .6rem;overflow-x:auto;-webkit-overflow-scrolling:touch">
        <button class="btn btn-primary btn-sm" onclick="openAddProductModal()" style="white-space:nowrap;flex-shrink:0">
        ...
        <button class="btn btn-sm" onclick="openScanModal()" style="white-space:nowrap;flex-shrink:0">
          <svg viewBox="0 0 256 256" fill="currentColor" width="12" height="12"><path d="M208,56H180.28L166.65,35.56A8,8,0,0,0,160,32H96a8,8,0,0,0-6.65,3.56L75.71,56H48A24,24,0,0,0,24,80V192a24,24,0,0,0,24,24H208a24,24,0,0,0,24-24V80A24,24,0,0,0,208,56Zm8,136a8,8,0,0,1-8,8H48a8,8,0,0,1-8-8V80a8,8,0,0,1,8-8H80a8,8,0,0,0,6.66-3.56L100.28,48h55.43l13.63,20.44A8,8,0,0,0,176,72h32a8,8,0,0,1,8,8ZM128,88a44,44,0,1,0,44,44A44.05,44.05,0,0,0,128,88Zm0,72a28,28,0,1,1,28-28A28,28,0,0,1,128,160Z"/></svg>
          סריקת תמונה
        </button>
        <button class="btn btn-sm" onclick="openExamineModal()" style="white-space:nowrap;flex-shrink:0">
          <svg viewBox="0 0 256 256" fill="currentColor" width="12" height="12"><path d="M229.66,218.34l-50.07-50.07A88.11,88.11,0,1,0,168,229.66,87.59,87.59,0,0,0,179.59,218Zm-137-30.46a72,72,0,1,1,51.83,21.87A71.52,71.52,0,0,1,92.66,187.88Zm128.22,38.45a8,8,0,0,1-11.31,0l-27.84-27.83a96.5,96.5,0,0,0,11.31-11.31l27.84,27.83A8,8,0,0,1,220.88,226.33Z"/></svg>
          בחני מוצר
        </button>
        <button class="btn btn-sm" onclick="openCompareModal()" style="white-space:nowrap;flex-shrink:0">
          <svg viewBox="0 0 256 256" fill="currentColor" width="12" height="12"><path d="M230.14,58.87A8,8,0,0,0,224,56H175.4L151.58,18.35a8,8,0,0,0-13.16,0L114.6,56H32A8,8,0,0,0,24,67.2L56,165.07V200a16,16,0,0,0,16,16H184a16,16,0,0,0,16-16V165.07l32-97.87A8,8,0,0,0,230.14,58.87ZM128,32.65,142.6,56H113.4ZM72,99a24,24,0,0,1,0-48Zm112,101H72V176H184Z"/></svg>
          השוואה
        </button>
      </div>

      <!-- Row 3: Library consult bar -->
      <div id="library-consult-bar" style="display:none;align-items:center;justify-content:space-between;
           background:rgba(176,152,144,.12);border-radius:var(--r-sm);padding:.55rem .85rem;
           margin:0 1rem .5rem;gap:.7rem">
        <span style="font-size:.73rem;color:var(--text-soft);line-height:1.4">האם הספרייה שלך מאוזנת?</span>
        <button class="btn btn-sm" onclick="openLibraryConsult()" style="white-space:nowrap;flex-shrink:0">
          ייעוץ ✦
        </button>
      </div>

      <!-- Row 4: Filter chips by category -->
      <div class="filter-bar" id="product-filter-bar" style="padding-right:1rem;padding-left:1rem">
        <button class="filter-chip active" data-cat="all" onclick="filterProducts('all',this)">הכל</button>
        <button class="filter-chip" data-cat="active" onclick="filterProducts('active',this)">פעיל <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="cleanser" onclick="filterProducts('cleanser',this)">ניקוי <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="toner" onclick="filterProducts('toner',this)">טונר <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="serum" onclick="filterProducts('serum',this)">סרום <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="moisturizer" onclick="filterProducts('moisturizer',this)">קרם לחות <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="spf" onclick="filterProducts('spf',this)">קרם הגנה <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="retinoid" onclick="filterProducts('retinoid',this)">רטינול <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="exfoliant" onclick="filterProducts('exfoliant',this)">פילינג <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="treatment" onclick="filterProducts('treatment',this)">טיפול <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="mask" onclick="filterProducts('mask',this)">מסכה <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
        <button class="filter-chip" data-cat="oil" onclick="filterProducts('oil',this)">שמן <span class="chip-count" style="font-size:.6rem;opacity:.7"></span></button>
      </div>

      <div id="products-list" style="padding:0 1rem"></div>
    </div>
    ...
    <div class="modal-row">
      <div class="modal-field">
        ...
        <select class="modal-select" id="mp-category">
          ...
          <option value="cleanser">ניקוי</option>
          <option value="toner">טונר</option>
          <option value="exfoliant">פילינג</option>
          <option value="serum">סרום</option>
          <option value="eye_care">קרם עיניים</option>
          <option value="moisturizer">קרם לחות</option>
  ...
  <div class="modal-sheet">
    ...
    <div style="text-align:center;margin-bottom:.8rem">
      <div style="display:flex;gap:.5rem;justify-content:center">
        ...
        <label class="btn" style="display:inline-flex;align-items:center;gap:.4rem;cursor:pointer">
          📷 צלמי עכשיו
          <input type="file" accept="image/*" capture="environment" multiple style="display:none" onchange="scanFileSelected(this)">
        </label>
      </div>
    </div>
...
<div id="modal-add-step" class="modal-backdrop hidden" onclick="if(event.target===this)closeModal('modal-add-step')">
  <div class="modal-sheet">
    ...
    <div class="filter-bar" id="mas-filter-bar">
      ...
      <button class="filter-chip" onclick="masFilter('cleanser',this)">ניקוי</button>
      ...
      <button class="filter-chip" onclick="masFilter('moisturizer',this)">קרם לחות</button>
      ...
      <button class="filter-chip" onclick="masFilter('exfoliant',this)">פילינג</button>
      <button class="filter-chip" onclick="masFilter('eye_care',this)">עיניים</button>
    </div>
...
<div id="modal-examine" class="modal-backdrop hidden" onclick="if(event.target===this)closeModal('modal-examine')">
  <div class="modal-sheet" style="max-height:90dvh;display:flex;flex-direction:column">
    ...
    <div style="text-align:center;margin-bottom:.7rem">
      ...
      <label class="btn" style="display:inline-flex;align-items:center;gap:.4rem;cursor:pointer">
        📷 צלמי עכשיו
        <input type="file" accept="image/*" capture="environment" style="display:none" onchange="examineFileSelected(this)">
      </label>
    </div>
...
<!-- Modal: Adapt Routine to Skin Photo -->
<div id="modal-adapt-routine" class="modal-backdrop hidden" onclick="if(event.target===this)closeModal('modal-adapt-routine')">
  <div class="modal-sheet" style="max-height:88dvh;display:flex;flex-direction:column">
    <div class="modal-handle"></div>
    <p class="modal-title" id="adapt-routine-title">התאמת השגרה לעורך</p>
    <span class="modal-sub">העלי תמונה של פניך — AI יבחן את מצב העור ויציע התאמות</span>

    <div style="display:flex;gap:.5rem;justify-content:center;margin-bottom:.7rem">
      <label class="btn btn-primary" style="display:inline-flex;align-items:center;gap:.4rem;cursor:pointer">
        <svg viewBox="0 0 256 256" fill="currentColor" width="14" height="14">
          <path d="M208,56H180.28L166.65,35.56A8,8,0,0,0,160,32H96a8,8,0,0,0-6.65,3.56L75.71,56H48A24,24,0,0,0,24,80V192a24,24,0,0,0,24,24H208a24,24,0,0,0,24-24V80A24,24,0,0,0,208,56Zm8,136a8,8,0,0,1-8,8H48a8,8,0,0,1-8-8V80a8,8,0,0,1,8-8H80a8,8,0,0,0,6.66-3.56L100.28,48h55.43l13.63,20.44A8,8,0,0,0,176,72h32a8,8,0,0,1,8,8ZM128,88a44,44,0,1,0,44,44A44.05,44.05,0,0,0,128,88Zm0,72a28,28,0,1,1,28-28A28,28,0,0,1,128,160Z"/>
        </svg>
        גלריה
        <input type="file" id="adapt-file" accept="image/*" style="display:none" onchange="adaptFileSelected(this)">
      </label>
      <label class="btn" style="display:inline-flex;align-items:center;gap:.4rem;cursor:pointer">
        <svg viewBox="0 0 256 256" fill="currentColor" width="14" height="14">
          <path d="M208,56H180.28L166.65,35.56A8,8,0,0,0,160,32H96a8,8,0,0,0-6.65,3.56L75.71,56H48A24,24,0,0,0,24,80V192a24,24,0,0,0,24,24H208a24,24,0,0,0,24-24V80A24,24,0,0,0,208,56Zm8,136a8,8,0,0,1-8,8H48a8,8,0,0,1-8-8V80a8,8,0,0,1,8-8H80a8,8,0,0,0,6.66-3.56L100.28,48h55.43l13.63,20.44A8,8,0,0,0,176,72h32a8,8,0,0,1,8,8ZM128,88a44,44,0,1,0,44,44A44.05,44.05,0,0,0,128,88Zm0,72a28,28,0,1,1,28-28A28,28,0,0,1,128,160Z"/>
        ...
        צלמי עכשיו
        <input type="file" accept="image/*" capture="user" style="display:none" onchange="adaptFileSelected(this)">
      </label>
    </div>

    <img id="adapt-preview" style="display:none;width:100%;max-height:160px;object-fit:cover;border-radius:var(--r-sm);margin-bottom:.6rem">
    <div id="adapt-result" style="flex:1;overflow-y:auto"></div>

    <div class="modal-actions" style="margin-top:.7rem">
      <button class="btn btn-primary" onclick="runAdaptRoutine()">נתחי והתאימי ✦</button>
      <button class="btn" onclick="closeModal('modal-adapt-routine')">ביטול</button>
    </div>
  </div>
</div>
