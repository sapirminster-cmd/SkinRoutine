...
כללי סינון — החל לפני הכל:
- כלולי רק מוצרים שיש להם "morning" ב-timeOfUse
- מוצר עם "night" בלבד — אסור לחלוטין בשגרת בוקר
- רטינואיד/retinoid — אסור בבוקר ללא יוצא מן הכלל
- AHA/BHA/exfoliant — אסור בבוקר אלא אם timeOfUse כולל "morning" במפורש

סדר שלבים (רק מקטגוריות הקיימות): קלנזר → טונר → exfoliant (אם בוקר) → סרום → עיניים → מוסטורייזר → SPF
- מוצר אחד לכל קטגוריה
- SPF חייב להיות האחרון תמיד
- אל תמציא שלבים שלא קיימים בספרייה
...
// ─── 4. Build night cycle ─────────────────────────────────────
async function aiBuildNightCycle(userGoal = '') {
  const products = DB.getProducts().filter(p => p.active);
  if (!products.length) throw new Error('אין מוצרים בספרייה');
  const list = products.map(p =>
    `ID:${p.id} | ${p.brand?p.brand+' ':''}${p.name} | ${p.category} | ${(p.timeOfUse||[]).join('+')}`
  ...
  const goalCtx = userGoal
    ? `המשתמשת רוצה: ${userGoal}`
    : 'בני מחזור skin cycling קלאסי המתאים למוצרים הזמינים.';
...
- כלולי רק מוצרים עם "night" ב-timeOfUse
- מוצר עם "morning" בלבד — אסור לחלוטין
- AHA/BHA ורטינול — אסור באותו לילה בשום מקרה
- כל לילה שמכיל AHA/BHA לא יכיל רטינול, וכל לילה עם רטינול לא יכיל AHA/BHA
- לילות התאוששות — קרם לחות + מחסום בלבד, ללא חומצות
- אם אין מוצרי רטינול/exfoliant — בני מחזור לחות בלבד
- אל תכלולי מוצר שלא מופיע ברשימה
...
async function aiChat(messages) {
  const products = DB.getProducts().filter(p=>p.active);
  const prods    = products.map(p=>`${p.name}(${p.category},${(p.timeOfUse||[]).join('+')})`).join(', ');
  const system   = [
    'את יועצת טיפוח עור מומחית. עני בעברית, בסגנון חמות ומקצועי.',
    _profileCtx(),
    prods ? `מוצרי המשתמשת: ${prods}` : '',
    'עקרונות עקביות שחובה לשמור עליהם בכל המלצה:',
    '- רטינואיד/רטינול — לילה בלבד, אסור בבוקר',
    '- AHA/BHA — לא לשלב עם רטינול באותו לילה',
    '- SPF — בוקר בלבד, שלב אחרון',
    '- אל תסתרי המלצות שנתת קודם באותו שיח',
  ].filter(Boolean).join('\n');
